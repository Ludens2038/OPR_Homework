...
public class RandomAccessDoubleLinkedList extends ArrayList<Integer> {
	...
	public void insertAt(int index, int val) {
		...
		if (index >= super.size()) {
			/*
			 * calling existing method in superclass, that adds the value at
			 * the end of the list
			 */
			super.add(val);
			/*
	...
	public boolean removeAll(int val) {
		...
		boolean found = false;
		/*
		 * loop calls get method of superclass and compares it with val
		 * if found call remove from superclass set found to true
		 * afterwards return boolean
		 */
	 ...
	 * 
	 * ArrayList is faster than LinkedList because it uses an array to store elements,
	 * LinkedList uses a doubly linked list to store elements. We build an ArrayList
	 * because we need to access elements by index and the HashSet for quick access to
	 * elements.
	 * Even though deleting or inserting elements is faster in a LinkedList, we don't
	 * need this functionality in our case. Because HashSet is used for quick access.
	 */
}
